        ...
        <div class="card card-hover cursor-pointer flex flex-col overflow-hidden p-0" onclick="openApp('projects','${p.id}')">
          ...
          <div class="p-3 flex-1 flex flex-col">
            ...
            <div class="flex gap-2 mt-2">
              ...
              <span class="ml-auto text-[11px] text-violet-300">View →</span>
            </div>
          </div>
        </div>
      `).join('')}
...
function appContact(){
  ...
  <div class="p-6 flex flex-col items-center gap-4 max-w-md mx-auto w-full">
    ...
    <p class="text-sm opacity-60 text-center">Open to internships, junior roles & collaboration.</p>
    <div class="w-full space-y-3">
      <a href="mailto:${p.email}" class="flex items-center gap-3 p-3 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10">
        <span class="w-10 h-10 rounded-lg bg-blue-500/20 grid place-items:center"><i data-lucide="mail" class="w-5 h-5 text-blue-400"></i></span>
        <span class="flex-1"><span class="text-xs opacity-60">Email</span><br><b class="text-sm">${p.email}</b></span>
        <i data-lucide="arrow-up-right" class="w-4 h-4 opacity-40"></i>
      </a>
      <a href="${p.github}" target="_blank" class="flex items-center gap-3 p-3 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10">
        <span class="w-10 h-10 rounded-lg bg-white/10 grid place-items:center"><i data-lucide="github" class="w-5 h-5"></i></span>
        <span class="flex-1"><span class="text-xs opacity-60">GitHub</span><br><b class="text-sm">github.com/0xAbhi13</b></span>
        <i data-lucide="arrow-up-right" class="w-4 h-4 opacity-40"></i>
      </a>
      <a href="${p.linkedin}" target="_blank" class="flex items-center gap-3 p-3 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10">
        <span class="w-10 h-10 rounded-lg bg-sky-500/20 grid place-items:center"><i data-lucide="linkedin" class="w-5 h-5 text-sky-400"></i></span>
        <span class="flex-1"><span class="text-xs opacity-60">LinkedIn</span><br><b class="text-sm">linkedin.com/in/0xAbhi13</b></span>
        <i data-lucide="arrow-up-right" class="w-4 h-4 opacity-40"></i>
      </a>
      <div class="flex items-center gap-3 p-3 bg-white/5 border border-white/10 rounded-xl">
        <span class="w-10 h-10 rounded-lg bg-white/10 grid place-items:center">◐</span>
        <span class="flex-1"><span class="text-xs opacity-60">Location</span><br><b class="text-sm">${p.location}</b></span>
      </div>
    </div>
    <button onclick="openApp('resume')" class="mt-2 px-6 py-2.5 bg-white/10 border border-white/10 text-white/70 rounded-full text-sm font-semibold flex items-center gap-2 hover:bg-white/15">
      <i data-lucide="clock" class="w-4 h-4"></i> Resume — Coming Soon
    </button>
  </div>`;
}
